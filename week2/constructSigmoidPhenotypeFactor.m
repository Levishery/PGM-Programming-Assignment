...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
geneNum = length(alleleWeights);
alleleNum = [];
for i = 1:length(alleleWeights)
    alleleNum = [alleleNum length(alleleWeights{i})];
end
phenotypeFactor.var = [phenotypeVar geneCopyVarOneList' geneCopyVarTwoList'];
phenotypeFactor.card = [2  alleleNum alleleNum];
...
for i =  1:prod(phenotypeFactor.card)
    f = 0;
    for  j = 1:geneNum
        f = f+alleleWeights{j}(assignments(i,1+j))+alleleWeights{j}(assignments(i,1+j+alleleNum(1)));
    end
